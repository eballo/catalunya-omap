...
const cssSrc = path.join(ROOT, 'web/css/catalunya-omap.css');
...
for (const dest of [path.join(PLUGIN_CSS, 'catalunya-omap.min.css'), path.join(ROOT, 'dist/production/catalunya-omap.min.css')]) {
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    fs.writeFileSync(dest, cssContent);
    console.log(`CSS → ${path.relative(ROOT, dest)}`);
}

// MarkerCluster CSS — always overwrite
const clusterSrc = path.join(ROOT, 'node_modules/leaflet.markercluster/dist');
for (const name of ['MarkerCluster.css', 'MarkerCluster.Default.css']) {
    const src  = path.join(clusterSrc, name);
    const dest = path.join(PLUGIN_CSS, name);
